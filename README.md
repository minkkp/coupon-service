...
> Redis와 Lua를 활용해  
> 대규모 동시 요청 환경에서도  
> 한정 수량의 선착순 쿠폰을 정확하게 발급하고,  
> 장애·운영 대응까지 고려한 시스템을 구현한 토이 프로젝트입니다.

---

## 📌 프로젝트 목적

- 대규모 트래픽 환경에서 한정 수량 쿠폰을 선착순으로 정확하게 발급
- 과발급 / 중복 발급 방지
...
- DB 비동기 저장으로 발급 API 응답 경로에서 DB 제거
...
- **Redis**
  ...
  - Lua 스크립트로 중복 체크 + 재고 감소를 단일 연산으로 처리
...
동시성 문제 해결, 성능 최적화, 장애·운영 대응까지 고려한  
실무 중심의 설계 경험을 목표로 했습니다.
