...
### Redis 장애
- Redis 오류 발생 시 즉시 실패 처리 (Fail-Fast)
...
- 만료 시간(TTL) 설정에 따라 이벤트 자동 종료처리 (2차 안전장치)

---

## 🗂️ ERD

해당 프로젝트에서는 쿠폰 발급 성능과 정합성을 우선시하여  
...
- 쿠폰발급이력(CouponIssue)는 성공 이력만 저장,  
  쿠폰 발급 여부 판단은 Redis에서 처리
